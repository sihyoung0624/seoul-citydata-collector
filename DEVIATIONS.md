...
- **중간 점검 (2026-09-11, 운영 31일차) — A 효과 확인, B 효과 미미**:
  - [A] 예측 확보: base_time **하루 9~14개** (9/5~11 실측) — 적용 전 1~2개에서 5~7배 회복, 판정 기준(5개+) 상회 ✅
  - [B] 수집 횟수: 하루 success 629~1,128건 (5~9회/일) — 기준(2,000건) 미달, 정각 회피 효과는 제한적 (예상대로)
  - 실행 실패 0. 9/7 월요일 백업 정기 실행 성공 (스케줄 자체 지연은 있었으나 자동 수행됨)
  - DB 213MB (43%). raw 첫 삭제는 9/14 월요일 백업에서 발생 예정 (8/11 데이터가 그때 30일 경과)
  - 결론: 핵심 가치(예측-실측 대조)는 회복. 9/17 자동 점검에서 "현상 유지 vs Edge Function 이전" 선택지 제시 예정

- **✅ 권장안 A+B 적용 (2026-09-03, 사용자 승인)**:
  - [A] `parser.js` `shouldSaveForecast`: 분 00~09 조건 제거, 유효한 시각이면 매 실행 저장.
    지시서 4-3 '예측 정시 1회' 규칙의 공식 완화 — 규칙의 전제(10분 간격 실행)가 무너져
    핵심 가치(예측-실측 대조)를 훼손했기 때문. 중복은 uq_forecast + ignore-duplicates가 차단
  - [B] `collect.yml` cron 정각 회피: `*/10` → `3-53/10`, 통합 `0 * * * *` → `7 * * * *`
    (COLLECT_MODE 판별 문자열도 `7 * * * *`로 동기화)
  - README 6단계 예측 확인 문구 갱신. [C](Supabase Edge Function 이전)는 2주 관찰 후 판단 — 2026-09-17 자동 점검 예약
- **점검 (2026-09-03, 운영 23일차) — 미회복 판정 + 대안 검토 (구현 전, 사용자 승인 대기)**:
  - **판정: 미회복.** 기준(하루 success 2,000건 = 인구 수집 16회/일 이상) 미달.
    실측 9/1 877건 · 9/2 876건 (인구 수집 약 7회/일, 실효 간격 3~3.5시간).
    8/31 관측(하루 585건 · 4~5회) 대비 소폭 개선됐으나 설계값(144회/일)과는 여전히 20배 차이
  - 실행 자체는 전부 성공(gh run list 최근 30건 failure 0). GitHub 무료 스케줄 큐 혼잡이 원인이며 우리 쪽 고장 아님
  - **핵심 발견 — 예측(base_time) 확보가 하루 1~2개로 붕괴**: `parser.js`의 `shouldSaveForecast`가
    스냅샷 PPLTN_TIME의 분이 00~09일 때만 예측을 저장한다. 실행이 정시 근처에 오는 것을 전제한 규칙인데,
    지금은 실행 시각이 무작위라 7회 중 1회만 조건을 만족한다.
    실측 base_time: 8/27 2개 → 8/28·8/30·8/31 각 1개 → 9/2 2개 → 9/3 1개 (8월 중순 정상기는 하루 8개+).
    즉 수집 간격 저하보다 **이 저장 조건이 예측-실측 대조라는 핵심 가치를 더 크게 훼손**하고 있다
  - DB 159MB (테이블 합계: raw 96MB / snapshot 27MB / forecast 24MB / log 12MB). 30일 정리 도입 후 안정

  - **대안 비교** (평가 기준: 무료 유지 / 비개발자 관리 단순성 / 기존 코드 재사용 / 이중 수집 시 중복 안전성):

    | 대안 | 무료 | 단순성 | 코드 재사용 | 중복 안전 | 효과 |
    |---|---|---|---|---|---|
    | ① cron 오프셋 (`3-53/10`) | 유지 | 매우 높음(1줄) | 그대로 | 해당 없음 | 소~중 (불확실) |
    | ② Supabase Edge Function + pg_cron | 유지(무료 티어 내) | 중간 | 높음(Deno·fetch 그대로) | 안전 | 큼 |
    | ③ Cloudflare Workers Cron | 유지(계정 신규 필요) | 낮음 | 높음 | 안전 | 가장 큼 |
    | ④ 현상 유지 | 유지 | 최고 | — | — | 없음 |

    - **① cron 오프셋**: 정시(분 00)는 전 세계 GitHub cron이 가장 몰리는 시각이므로 정시를 피하는 것 자체는
      소폭 효과가 기대되나, 원인이 큐 전반의 혼잡이라 근본 해결은 아님. 비용 0, 되돌리기 쉬움
    - **② Supabase Edge Function + pg_cron**: GitHub 스케줄러 의존을 완전히 제거. `pg_cron`·`pg_net`은 무료 티어 사용 가능,
      Edge Function 무료 호출 한도(월 50만) 대비 10분 주기는 월 4,320회로 여유. 키·스케줄·DB가 Supabase 한 곳으로 모여 관리 지점이 줄어듦.
      **제약**: 무료 Edge Function의 실행 시간 상한(약 150초)에 121곳 순차 수집(현재 약 2분)이 아슬아슬 → 121곳을 2~3배치로 쪼개는 수정 필요.
      배포는 Supabase MCP로 가능해 사용자가 CLI를 설치할 필요 없음
    - **③ Cloudflare Workers Cron**: 스케줄 정확도가 가장 높음(거의 정시). 의존성 0·ESM·fetch 구조라 코드는 거의 그대로 이식 가능.
      **제약**: Cloudflare 계정 신규 개설 + Wrangler CLI로 사용자가 직접 배포해야 함(내가 대신 배포 불가), 무료 플랜 Worker CPU 시간 제한 확인 필요,
      관리 지점이 GitHub·Supabase·Cloudflare 3곳으로 늘어남 — 비개발자 운영에는 부담
    - **④ 현상 유지**: 스냅샷은 하루 850건씩 계속 쌓여 혼잡도 시계열의 큰 그림은 유지되나,
      위 '핵심 발견'대로 예측 확보가 하루 1~2개라 예측-실측 대조는 사실상 성립하지 않음. 그대로 두는 것은 권장하지 않음

  - **권장안 (단계적, 승인 대기)**:
    1. **[A] 예측 저장 창 완화 — 최우선.** `shouldSaveForecast`의 분 00~09 조건을 제거해 매 실행마다 예측을 저장한다.
       `uq_forecast unique (area_cd, base_time, fcst_time)` + ON CONFLICT DO NOTHING이 이미 있어 중복은 자동으로 걸러진다.
       코드 1줄, 스케줄러 변경 불필요, base_time 확보가 하루 1~2개 → 6~7개로 즉시 회복.
       *트레이드오프*: base_time이 정시에 정렬되지 않음(예 09:35) → 분석 시 시각 단위 반올림 필요.
       예측 테이블 증가는 하루 약 1.5천 → 1만 행으로, 8월 중순 정상기(하루 6천~1.2만)와 같은 수준이라 용량 계획 내.
       *지시서 이탈*: 지시서의 '분 00~09 규칙'을 완화하는 변경이므로 사용자 승인 필요
    2. **[B] cron 오프셋 적용** — `*/10` → `3-53/10`, 정시 혼잡 회피. 비용·리스크 거의 0
    3. **[C] 2주 관찰 후에도 수집 횟수가 부족하면 ② Supabase Edge Function 이전** 검토.
       기존 GitHub Actions를 남긴 채 병행 가동해도 중복 안전(모든 테이블에 UNIQUE + ON CONFLICT DO NOTHING)하므로
       한 번에 갈아타지 않고 안전하게 이행 가능. ③은 관리 지점 증가 때문에 후순위
