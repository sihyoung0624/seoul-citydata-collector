...
- **점검 (2026-08-31, 운영 20일차) — cron 스로틀링 심화 관측**:
  - 최근 3일 수집 성공 1,754건 (하루 약 585건 ≈ 인구 수집 4~5회/일). 8월 중순(40회+/일) 대비 크게 감소
  - 실행 자체는 전부 성공(실패 0). GitHub 무료 스케줄 큐 혼잡으로 실행 간격이 약 2~3시간까지 벌어진 것
  - 우리 쪽 원인 아님·조치 불가. 데이터는 계속 쌓이는 중 (snapshot 70,519 / forecast 115,267 / base_time 108개)
  - DB 171MB (34%). 8/31 월요일 백업은 스케줄 지연으로 수동 실행하여 성공 (8/30까지 백업 완료)
  - 참고: 이 정도 간격에도 시계열 가치는 유지되나, 시간대별 해상도가 낮아진 기간으로 분석 시 표시 필요
